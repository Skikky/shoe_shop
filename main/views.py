...
def index(request):
    ...
    # Recupera dinamicamente i brand dal database insieme ai modelli associati.
    brands_with_models = Brand.objects.prefetch_related('models').all()
...
def catalog_page(request, browse_filter="all"):
    ...
    # Recupera tutti i modelli di scarpe dal database
    all_models = ShoeModel.objects.all().order_by('model')

    # Controlla se ci sono filtri min e max price nell'URL GET, altrimenti imposta valori predefiniti
    ...
    # Inizializza category e display_shoes come valori vuoti
    category = ""
    display_shoes = ShoeColor.objects.none()

    # Se browse_filter è "all", mostra tutte le scarpe
    if browse_filter.lower() == "all":
        category = "All Sneakers"
        display_shoes = ShoeColor.objects.filter(model__price__gte=min_price, model__price__lte=max_price)
    else:
        # Verifica se browse_filter corrisponde a un nome di brand (ignorando maiuscole e minuscole)
        for brand in all_brands:
            if brand.name.lower() == browse_filter.lower():
                category = brand.name
                display_shoes = ShoeColor.objects.filter(model__brand=brand, model__price__gte=min_price, model__price__lte=max_price)
                break

        # Se browse_filter è un ID numerico, cerca un modello specifico
        if category == "":
            try:
                model = ShoeModel.objects.get(id=int(browse_filter))
                category = model.model
                display_shoes = ShoeColor.objects.filter(model=model, model__price__gte=min_price, model__price__lte=max_price)
...
def add_shoe(request):
    ...
    # Verifica se colori e immagini sono forniti e hanno la stessa lunghezza
    if not colors or not images or len(colors) != len(images):
        error_message = "Errore: Assicurati di aver fornito una foto per ciascun colore inserito (torna indietro per editare)."
        return HttpResponse(error_message)

    # Verifica se la scarpa esiste già
    existing_shoe = ShoeSize.objects.filter(
        color__model__model=model_name,
        color__model__brand__name=brand_name,
        color__color__in=colors
    ).first()

    if existing_shoe:
        error_message = "Errore: Una scarpa con gli stessi attributi esiste già nel database (torna indietro per editare)."
        return HttpResponse(error_message)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # Crea colori e dimensioni
            colors_and_sizes = []
            sizes = [6.0, 6.5, 7.0, 7.5, 8.0, 8.5, 9.0, 9.5, 10.0, 10.5, 11.0, 11.5, 12.0]

            for color, image in zip(colors, images):
                new_color = ShoeColor.objects.create(color=color, image=image, model=model)
                sizes_for_color = [ShoeSize(size=size, inventory=5, quantity_sold=0, color=new_color) for size in sizes]
                colors_and_sizes.extend(sizes_for_color)

            ShoeSize.objects.bulk_create(colors_and_sizes)
    except IntegrityError as e:
        # Gestisci eccezioni di integrità del database
        error_message = "Errore di integrità del database: {}".format(str(e))
        return HttpResponse(error_message)
